...
import { Button, Card, Empty, ErrorAlert, Input, InputMonto, Label, LoadingOverlay } from './ui/index.js';
...
function FormExtra({ inicial, textoBoton = 'Agregar', onGuardar, onCancelar }) {
  ...
  async function guardar() {
    if (!titulo.trim()) {
      notificar('El extra necesita un título.');
      return;
    }
    setGuardando(true);
    try {
...
function FilaExtra({ extra, recargar }) {
  ...
  async function borrar() {
    const ok = await confirmar(`¿Borrar el extra "${extra.titulo}"? Esta acción no se puede deshacer.`);
    if (!ok) return;
    try {
...
export function Extras({ proyectoId }) {
  const { datos, error, cargando, recargar } = useCargar(() => api.get(`/proyectos/${proyectoId}/extras`), [proyectoId]);
